...
export default function KinkImageGen() {
  ...
  const getEnhancedPrompt = () => {
    ...
    if (selectedPromptAdds) {
      enhanced = `${enhanced}, ${selectedPromptAdds}`;
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGeneratedImage(data.imageUrl || 'https://picsum.photos/800/600?random=' + Date.now());
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="mt-6">
              <p className="text-sm mb-3">Kink Tags (optimised for anal/enema/food play)</p>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl aspect-video flex items-center justify-center">
            {generatedImage ? <img src={generatedImage} alt="Generated" className="max-h-full" /> : <p className="text-zinc-500">Image will appear here</p>}
          </div>
        </div>
      </div>
    </div>
